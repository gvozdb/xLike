...
if ($transport->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            $modelPath = MODX_CORE_PATH . 'components/xlike/model/';
            $modx->addPackage('xlike', $modelPath);

            $manager = $modx->getManager();
            $objects = array();
            $schemaFile = $modelPath . 'schema/xlike.mysql.schema.xml';
            ...
            foreach ($objects as $tmp) {
                ...
                unset($modx->map[$tmp]);
                $modx->loadClass($tmp, $modelPath . 'xlike/');
